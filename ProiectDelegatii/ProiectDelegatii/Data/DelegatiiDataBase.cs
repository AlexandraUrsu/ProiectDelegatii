...
namespace ProiectDelegatii.Data
{
    public class DelegatiiDataBase
    {
        ...
        //Document
        public Task<int> SaveDocumentAsync(Document doc)
        {
            if (doc.ID != 0)
            {
                return _database.UpdateAsync(doc);
            }
            else
            {
                return _database.InsertAsync(doc);
            }
        }

        public Task<int> DeleteDocumentAsync(Document doc)
        {
            return _database.DeleteAsync(doc);
        }

        public Task<List<Document>> GetDocumentAsync()
        {
